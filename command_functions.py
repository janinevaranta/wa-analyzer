...
def help(*arg):
    ...
    "frame": Print out pandas Dataframe.
...
def font_size(*arg):
    ...
    try:
        ...
        print("The font size must be a viable integer that is larger than 0.")
...
def messages_per_sender_bar(chat_df):
    message_count = (
        chat_df.groupby("sender")["message"].count().sort_values(ascending=False)
    )
...
def messages_per_sender_plot(chat_df):
    per_sender = chat_df.groupby([chat_df.date, chat_df.sender])["message"].count()
...
def total_daily_messages_plot(chat_df):
    ...
    idx_range = pd.date_range(date.head(1).index.item(), date.tail(1).index.item())
...
def find_words(chat_df):
    ...
    if word_list_in != "":
        ...
        found_words_df = found_words_df.sort_values(by="frequency", ascending=False)
    else:
    ...
    print("")
    print("Create a bar graph from the top 10? y/n")
    answer = input(">")
    if answer.lower() == "y":
        plt.figure(figsize=(14, 8))

        plt.bar(found_words_df.head(10).index, found_words_df.head(10).values)
...
def leet(chat_df):
    ...
    result_leet = result_time.loc[result_time["message"].str.lower() == "leet"]
    print(result_leet)
    if result_leet.empty == True:
        pass
    else:
        leet_count = (
            result_leet.groupby("sender")["message"]
            .count()
...
def word_occurances(chat_df):
    ...
    n = (
        chat_df.loc[match_index.index]
        .groupby("sender")[["message"]]
        .count()
...
def find_date(chat_df):
    ...
    try:
        ...
        print("Invalid date format! Please use the following format: yyyy-mm-dd.")
